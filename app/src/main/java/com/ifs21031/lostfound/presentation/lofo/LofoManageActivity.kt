...
import com.ifs21031.lostfound.databinding.ActivityLofoManageBinding
import com.ifs21031.lostfound.presentation.ViewModelFactory

class LofoManageActivity : AppCompatActivity() {
    private lateinit var binding: ActivityLofoManageBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityLofoManageBinding.inflate(layoutInflater)
        ...
        if (isAddTodo) {
            ...
            val delcomTodo = when {
                Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU -> {
                    intent.getParcelableExtra(KEY_TODO, DelcomLofo::class.java)
                }

                else -> {
                    @Suppress("DEPRECATION")
                    intent.getParcelableExtra<DelcomLofo>(KEY_TODO)
                }
            }
            if (delcomTodo == null) {
        ...
        binding.apply {
            ...
            btnTodoManageSave.setOnClickListener {
                ...
                if (title.isEmpty() || description.isEmpty()) {
                    AlertDialog.Builder(this@LofoManageActivity).apply {
                        setTitle("Oh No!")
    ...
    private fun observePostTodo(title: String, description: String) {
        viewModel.postLofo(title, description).observeOnce { result ->
            when (result) {
                is MyResult.Loading -> {
                    showLoading(true)
                }
    ...
    private fun manageEditTodo(todo: DelcomLofo) {
        binding.apply {
